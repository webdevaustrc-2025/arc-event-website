...
// Specification Rule: RENAME mockUsers to FALLBACK_USERS and keep original array
...
export default function AdminUsersPage() {
  ...
  // 1. Fetch Users logic with Dual-Source Pattern Fallback
  const fetchUsers = async () => {
    setLoading(true);
    try {
      const dbData = await adminFetch("/api/admin/users");

      if (dbData && dbData.length > 0) {
        // Flatten relational DB rows to match table schema expectations
        const mappedUsers = dbData.flatMap((user: any) => {
          if (user.registrations && user.registrations.length > 0) {
            return user.registrations.map((reg: any) => ({
              id: user.id,
              registrationId: reg.id,
              name: user.name || "Anonymous",
              email: user.email,
              team: user.team || "Independent",
              role: user.role === "admin" ? "Admin" : reg.role || "Member",
              status: reg.status.charAt(0).toUpperCase() + reg.status.slice(1), // Capitalize
              segment: reg.segment?.name || "N/A",
            }));
          }
          // If a user has no active registrations, show flat placeholder values
          return [
            {
              id: user.id,
              registrationId: null,
      ...
      console.error("Live fetch failed, falling back to dummy data:", error);
      setUsers(FALLBACK_USERS);
    } finally {
      setLoading(false);
    }
  ...
  // 2. Action Handler for Registration approvals/rejections
  const handleUpdateRegistrationStatus = async (
    ...
    try {
      ...
      fetchUsers(); // Re-fetch list to update table cells
    } catch (error) {
  ...
  // 3. Filtering logic executed on dynamically fetched active data state
      ...
      <div className={`p-6 rounded-2xl border ${cardBg}`}>
        ...
        <div className="overflow-x-auto rounded-xl border border-transparent">
          <table className={`w-full text-left border-collapse ${textColor}`}>
            ...
            <tbody>
              {loading
                ? // Elegant loading skeletons displayed during database loading state
                ...
                : filteredUsers.map((user) => (
                    ...
                    >
                      ...
                      <td className="p-4 text-right">
                        {/* Approved/Rejected interactive buttons strictly for registrations in Pending status */}
                        {user.status === "Pending" && user.registrationId ? (
                          <div className="flex items-center justify-end gap-2">
                            <button
                              ...
                              className="p-1.5 rounded-lg bg-[#588157]/15 text-[#a3b18a] hover:bg-[#588157]/30 transition-all border border-[#588157]/30"
                            ...
                            <button
                              ...
                              className="p-1.5 rounded-lg bg-red-500/15 text-red-400 hover:bg-red-500/30 transition-all border border-red-500/30"
